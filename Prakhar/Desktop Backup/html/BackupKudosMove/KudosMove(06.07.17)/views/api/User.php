...
class User extends REST_Controller {
    ...
    //      public function secBookingCron_get(){


    //     // $cancel_booking = $this->User_model->customCancel_cron();
    //        $currDate = date('Y-m-d H:i:s');
    //        $getbookings = $this->db->query(" SELECT * FROM tbl_bookingRequests WHERE '".$currDate."' > DATE_ADD(date_created, INTERVAL 20 SECOND) and is_accepted = 0 and is_started = 0 and is_completed = 0 and is_cancelled = 0 ")->result();

    //       foreach ($getbookings as  $bookingvalue) {


    //        $rType = 1;
    //        $catType =  $bookingvalue->categoryType;
    //        $getDrivers = $this->User_model->getLocationNew($bookingvalue->pickup_lat,$bookingvalue->pickup_long,$rType,$catType);
    //          $pushData['message'] = "You have recieved a request for new task";
    //          $pushData['action'] = "new move";
    //          $pushData['req_id'] = $bookingvalue->id;
    //          $pushData['is_quote'] = $bookingvalue->is_quote;
    //          $pushData['value'] = $bookingvalue->value;
    //          $pushData['spMessage'] = 'Do you want to send proposal on it?';

    //        foreach ($getDrivers as $value) {

    //          $check_Membership = $this->User_model->getMembership_check($value->id);
    ...
    public function customerRating_post(){
        ...
        $customResponse = $this->User_model->insert_data('tbl_appRatings',$insertArray);
          if(empty($ratingResponse)){
    ...
    public function requestDetails_post(){                   // get details regarding particular request
        ...
        $data[0]->avgRating = $ratingData->avg_r;
    ...
    public function requestAction_post(){                                //action once move is created
        ...
        if($type == 1){ // is_accepted by driver
            ...
            if ($bookingDetails->is_accepted==0 || $bookingDetails->is_started==0 || $bookingDetails->is_cancelled==1 || $bookingDetails->is_completed==1) {
                ...
                $afterHourCharge = $getUser[0]->afterHourCharges;
            ...
            if ($bookingDetails->is_started==1 || $bookingDetails->is_cancelled==1 || $bookingDetails->is_completed==1) {
                ...
                if($cancelUser_type == 1){      // is_cancelled by driver
                    ...
                    if(!empty($driver_id)){
                        ...
                        if($getBookingData[0]->is_accepted == 0){
                            ...
                            $uptDAta = $this->User_model->update_data('tbl_wallet',array('balance'=>$newAmount,'date_updated'=>date('Y-m-d H:i:s')),array('user_id'=>$driver_id));
                            $addtransArray = array(
