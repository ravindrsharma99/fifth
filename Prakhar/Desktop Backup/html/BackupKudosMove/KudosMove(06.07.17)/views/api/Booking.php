    ...
    public function quoteinfo(){
      if(isset($_GET['orderbook'])){
        ...
        $date_sp = $this->input->GET('datepick');
          $ex = explode(' ', $date_sp);
          $time = $ex[1].' '.$ex[2];
          $timer = date("H:i:s", strtotime($time));
 ...
 // }
  public function info($id){
    ...
    //if(isset($_GET['id'])){
      ...
      $rows = $this->User_model->get_move_info($u_id);
       // echo "<pre>";print_r($rows['show']->book_id);print_r($rows['show']->accepted_by);print_r($rows['show']->user_id);
       $rating_detail = array(
        'request_id'=>$rows['show']->book_id,
        'user_id'=>$rows['show']->user_id,
        'driver_id'=>$rows['show']->accepted_by,
        'rating'=> $this->input->post('rating')
        );

    //}
    if(isset($_POST['rate_submit'])){
     $this->User_model->insert_data('tbl_driverRatings',$rating_detail); 
      $rows = $this->User_model->get_move_info($u_id);
     }
        ...
        if($move['card_det'] == "empty"){
          ...
          $check['wallet_money'] = $this->User_model->wallet_val($amt);
            if($check['wallet_money'] == 1){
              $this->session->set_flashdata('msg','Task complete Successfully');
  ...
  public function rewiew_order(){
    if(isset($_SESSION['review_data']['category_id'])){
      $cat_id = $_SESSION['review_data']['category_id'];
      $sub_cat_id = $_SESSION['review_data']['subCategory_id'];
    }else{
      $cat_id = $_SESSION['review_quote_data']['category_id'];
      $sub_cat_id = $_SESSION['review_quote_data']['subCategory_id'];
    }
    $move['category'] = $this->User_model->get_data_review($cat_id,$sub_cat_id);
    $this->load->view('header',$move);
    // $this->load->view('header');
    $this->load->view('sidebar');
