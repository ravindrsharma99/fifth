   ...
   // unset($_SESSION['review_data']);
            ...
            echo  "
                    ...
                    <input type='hidden' name='type".$i."' value='".$type."' id='type".$i."'>
                      <div class='check_penal'>
                        <input type='checkbox' class='sp-minus fff' id='pri12".$i."' name='price".$i."' value='".$price."' >
                  ...
                  <script>
                    $('#pri12".$i."').change(function(event){
                        if (this.checked){
                            var chk_val = $('#pri12".$i."').val();
                            
                        } else {
                            var chk_val1 = $('#pri12".$i."').val();
    ...
    public function quoteinfo(){
      ...
      $stringg = implode(',',$dataaa);
        ...
        //$_SESSION['service_data'] = $service;
        // echo "<pre>";
        // print_r($_SESSION['review_data']);die;
...
// }
  ...
  public function pay(){
    if(isset($_POST['submit'])){
      ...
      $stripeAmount = ceil($cal_dedt * 100);
        ...
        if(empty($customer)){
          throw new Exception("The Stripe Token was not generated correctly");
            ...
            if(isset($_SESSION['review_data'])){
              $movee = $this->User_model->bok_order();
              if($movee == 1){
                $this->session->set_flashdata('msg','Booking Order Successfully');
                redirect('Booking/new_booking');
              }else{
              ...
              $movee1 = $this->User_model->bok_quote_order();
              if($movee1 == 2){
  ...
  if(isset($_POST['val'])){
    ...
    $way_cal = $wayPoint_charge * $inc;
      ...
      if(!empty(isset($dataS))){
         ...
         $dist = $dataS->routes[0]->legs[0]->distance->text;
