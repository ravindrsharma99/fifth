...
class User_model extends CI_Model {
    ...
    public function log($data){
    	$sel = $this->db->query("SELECT * FROM tbl_users Where email = '".$data['email']."' and password = '".$data['password']."' and user_type =0 ");
  		$rows = $sel->row();
  		$email = $rows->email;
  		if($email != $data['email']){
  			return "error";
    ...
    public function get_data_review($cat_id,$sub_cat_id){
      $get1 = $this->db->select('categoryName,tbl_categories.image,subCategoryName,(select subCategoryName from tbl_subCategory where id = '.$sub_cat_id.') as sub_cat_name,(select image from tbl_subCategory where id = '.$sub_cat_id.') as sub_cat_image,tbl_subCategory.image as imagee');
             $this->db->from('tbl_categories');
             $this->db->join('tbl_subCategory','tbl_subCategory.category_id = tbl_categories.id');
             $this->db->where('tbl_categories.id',$cat_id);
      $cat_name = $get1->get()->row();
      return $cat_name;
    }
   ...
   // print_r($data);
    ...
    public function actvte($email){
      ...
      $u_id = $slct11->row();
      $id = $u_id->id;
      $slct1  = $this->db->query("SELECT pickup_lat,dropOff_lat,pickup_long,dropOff_long,id,booking_date,booking_time,totalprice,id,path_wayPoints FROM `tbl_bookingRequests` WHERE ( (is_accepted=1 or is_started=1) and is_completed=0 ) and is_cancelled =0 and user_id = '".$id."' ORDER BY `id` DESC");
      $sngle = $slct1->row()->id;
    ...
    public function cmplte($email){
      ...
      $u_id = $slct11->row();
      $id = $u_id->id;
      $slct1  = $this->db->query("SELECT pickup_lat,dropOff_lat,extra_fare,afterHourCharges,booking_date,booking_time,pickup_long,dropOff_long,id,path_wayPoints,totalprice FROM `tbl_bookingRequests` WHERE  is_completed = 1 and user_id = '".$id."' ORDER BY `id` DESC");
      $sngle = $slct1->row()->id;
   ...
   public function acc_update1($dataa){
    ...
    public function get_move_info($u_id){
      $qry = $this->db->query("SELECT * FROM tbl_bookingRequests WHERE id = '".$u_id."' ");
      $ro = $qry->row();
      $data1=array(
        'show'=>$ro
      );
...
// print_r($this->db->last_query()); die;
  ...
  public function iosPush($pushData=null) {
   ...
   public function androidPush($pushData=null){
    ...
    //   return "2";
    // }
     public function bok_order($data,$aar){
      $txn_id = $aar['balance_transaction'];
      $user = $_SESSION['user_id'];
      //$dataa1 = $_SESSION['review_data'];
      //print_r($data);die;
       $detail = array(
        'user_id'=> $_SESSION['user_id'],
        'card_no'=> $data['card'],
    ...
    public function bok_quote_order($data,$aar){
      $txn_id = $aar['balance_transaction'];
      $user = $_SESSION['user_id'];
      $dataa = $_SESSION['review_quote_data'];
       $detail = array(
